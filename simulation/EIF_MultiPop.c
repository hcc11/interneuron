...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    mxTmp = mxGetField(prhs[3],0,"Ncell");
    Ncell=mxGetPr(mxTmp);
    m1 = mxGetN(mxTmp);
    m2 = mxGetM(mxTmp);
    Npop = m1*m2;
    ...
    m2 = mxGetM(mxTmp);
    if(m1*m2!=Npop)
        mexErrMsgTxt("Iapp should be 1xNpop");
    ...
    for(i=1;i<Nt && ns<maxns;i++){
        ...
        while(*iXspkInd<=i*dt && iXspike<Nsx){
            ...
            for(post_pop=0;post_pop<Npop;post_pop++){
                ...
                for(isyn=0;isyn<Nsyn;isyn++){
                    if(syntype[isyn]==0){
                        for(k=0;k<Kx[post_pop];k++)
                            Isynprime[postcell[k]*Nsyn+isyn]+=Jx[post_pop]*temp2[isyn];
                    }
                }
            }
            iXspike++;
        ...
        for(j=0;j<N;j++){
            ...
            if(refstate[j]<=0){
                ...
                v[j]+=fmax((Iapp[pre_pop]+Isyntot-gl[pre_pop]*(v[j]-Vleak[pre_pop])+gl[pre_pop]*DeltaT[pre_pop]*EXP((v[j]-VT[pre_pop])/DeltaT[pre_pop]))*dt/C[pre_pop],Vlb[pre_pop]-v[j]);}
            else{
                if(refstate[j]>1)
                    v[j]=Vth[pre_pop];
                else
