...
// going to be OU input
/* Combination of Poisson and Hetero Iapp - cannot run with Iapp = [1x4] because this code requires and depends on Iapp = [1xN]
*/
...
/*double generate_random_number() {
    double a = 0.0;
    double b = 1.0;
    return ((double)rand() / RAND_MAX) * (b-a) + a;  // random number between lower and upper
}*/

double normal_random_number(double mean, double stddev){
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  //  int *Ntref,Npop,Nx,*Nsum,*Ksum,Kxsum,Kmax,post_pop,pre_pop,k,j,i,N,Nt,m1,m2,maxns,ns,Nrecord,jj,nskiprecord,tempflag,Nsx, Nw;
    int *Ntref,Npop,Nx,*Nsum,*Ksum,Kxsum,Kmax,post_pop,pre_pop,k,j,i,N,Nt,m1,m2,maxns,ns,Nrecord,jj,nskiprecord,tempflag,Nsx, Nw, Nsrr;
    ...
    double meanOU, sigmaOU, tauOU;
    double *ouProcess;
    int popwithinput, val;
    double dW;
    double *recordedInput, *recordedOU, *recordedRandNum; 
    ...
    mxTmp = mxGetField(prhs[3],0,"meanOU");
    meanOU=mxGetPr(mxTmp)[0]; 
    mexPrintf("This is meanOU: %f \n",meanOU);

    mxTmp = mxGetField(prhs[3],0,"sigmaOU");
    sigmaOU=mxGetPr(mxTmp)[0]; 
    mexPrintf("This is sigmaOU: %f \n",sigmaOU);

    mxTmp = mxGetField(prhs[3],0,"tauOU");
    tauOU=mxGetPr(mxTmp)[0]; 
    mexPrintf("This is tauOU: %f \n",tauOU);

    mxTmp = mxGetField(prhs[3],0,"initialOU");
    ouProcess=mxGetPr(mxTmp); 
    m1 = mxGetN(mxTmp);
    m2 = mxGetM(mxTmp);
    mexPrintf("ouProcess initial size: %d %d \n",m1,m2);
    /**
    for(int i = 0; i < m2; i++){
        mexPrintf("%f\n",ouProcess[i]);
    } */
    ...
    popwithinput=mxGetPr(mxTmp)[0]; 
    mexPrintf("popwithinput: %d\n",popwithinput);

    srandom(time(NULL)); // set seed for OU process
    ...
    Ncell=mxGetPr(mxTmp); // Number of cells per population in recurrent layer
    m1 = mxGetN(mxTmp);
    m2 = mxGetM(mxTmp);
    Npop = m1*m2;
    //mexPrintf("This is Ncell, m1xm2:: %d %d \n",m1, m2);
    ...
    Jx=mxGetPr(mxTmp); //feedforward weight 
    m1 = mxGetN(mxTmp);
    m2 = mxGetM(mxTmp);
    if(m1*m2!=Npop)
    ...
    plhs[3] = mxCreateDoubleMatrix(1, Nt, mxREAL);
    recordedInput=mxGetPr(plhs[3]); // record OU voltage for a few neurons

    plhs[4] = mxCreateDoubleMatrix(1, Nt, mxREAL);
    recordedOU=mxGetPr(plhs[4]); // record OU for a few neurons

    // plhs[5] = mxCreateDoubleMatrix(1, Nt, mxREAL);
    // recordedInputNone=mxGetPr(plhs[5]); // record OU voltage for a few neurons

    plhs[5] = mxCreateDoubleMatrix(1, Nt, mxREAL);
    recordedRandNum=mxGetPr(plhs[5]); // record OU voltage for a few neurons
   ...
   // Isynrecord_noisyIapp=mxGetPr(plhs[3]); // record synaptic current
    ...
    // Nsum =[40000 44000 48000 50000]
    Nsum[0]=(int)Ncell[0];//set initial number of neurons to initial number of cells per population in reccurent layer
    mexPrintf("right here: i=%d, Nsum=%d \n",0,(int)Nsum[0]);
    for(i=1;i<Npop; i++){
        Nsum[i] = Nsum[i-1] + (int)Ncell[i];
        mexPrintf("i=%d, Nsum=%d \n",i,(int)Nsum[i]); 
   } 

    mexPrintf("popwithinput=%d, Nsum[popwithinput-2]=%d, Nsum[popwithinput-1]+1=%d \n",popwithinput,(int)(Nsum[popwithinput-2]),(int)(Nsum[popwithinput-1]+1)); 

   // Nsum results as vector: [Ncell[0], Ncell[0]+Ncell[1], Ncell[0]+Ncell[1]+Ncell[2],...]
    //tmp = (int)Nsum[2]-Nsum[1];
    //mexPrintf("Nsum(2)=%d, Nsum(3)=%d, tmp\n",(int)Nsum[1],(int)Nsum[2],tmp); 
  ...
  /*  mexPrintf("Kxsum=%d, Kmax=%d \n",Kxsum,Kmax);  */
    ...
    iRspkInd=&srr[0]; /* even index: spk times. odd index: neuron ID */ // neuron (ID) spks at time
    ...
    for(i=1;i<Nt && ns<maxns;i++){
        ...
        //if (  >= Nsum[popwithinput-2] && j < Nsum[popwithinput-1]){
        
        for(jj=Nsum[popwithinput-2];jj<Nsum[popwithinput-1];jj++){
            dW = sqrt(dt)*normal_random_number(0,1); // generate noise term
                // Ou value has index [jj-Nsum[popwithinput-2], subtract by Nsum[popwithinput-2]
            

            //mexPrintf("jj: %d, jj-idx: %d\n" jj, (int)(jj-Nsum[popwithinput-2]));
            ouProcess[jj-Nsum[popwithinput-2]] = ouProcess[jj-Nsum[popwithinput-2]] + (1/tauOU) *(meanOU - ouProcess[jj-Nsum[popwithinput-2]])*dt+(sigmaOU/tauOU)*dW; // update OU term, saves update of neuron j 
            

            if(jj==Nsum[popwithinput-2]){
               recordedOU[i-1]=ouProcess[jj-Nsum[popwithinput-2]]; // record OU input value
               recordedInput[i-1] = v[jj]; // record membrane potential at t = i-1
               recordedRandNum[i-1] = dW;
            }

        }
        
        /// could do a lopop over the size/num of cell inputs and then  read the specific one by index
        ...
        if (Nsrr > 0) {
          while(*iRspkInd<=i*dt && iRspike<Nsrr){
              ...
              rspike=(int)round((*iRspkInd)-1); //neuron ID for 
              //mexPrintf("\n %d",rspike);
              if(rspike<0 || rspike>=N){ // check within index of srr
              ...
              if (rspike <= Nsum[0]){
                post_pop = 0;
              } else if (rspike <= Nsum[1]) {
                  post_pop = 1;
              } else if (rspike <= Nsum[2]){
                  post_pop = 2;
              } else {
                  post_pop = 3;
              }
             ...
             ////////////////////////////////////////////////////              
              ...
              for(isyn=0;isyn<Nsyn;isyn++){ // update I current history wtih temp 2, Jrr established in Wrr loop
                if(syntype[isyn]==0){
                    Isynprime[rspike*Nsyn+isyn]+=Jext[post_pop]*temp2[isyn]; //
                  }
              }
            ...
            ////////////////////////////////////////////////////
              ...
              iRspike++; // update number of spks recorded
              iRspkInd++;
          }
        }
        ...
        for(j=0;j<N;j++){
            ...
            if(refstate[j]<=0){
                ...
                if ( j >= Nsum[popwithinput-2] && j < Nsum[popwithinput-1]){ //extra minus one because c starts at 0
                  //  mexPrintf("j value %d\n", j);
                 //   val = 1;
                   val =  j-Nsum[popwithinput-2];
                  //  val = 1*j;
                   //mexPrintf("val: %d , j: %d\n", val, j);
                    //dW = sqrt(dt)*normal_random_number(0,1); // generate noise term
                  //  dW = sqrt(dt)*normal_random_number(0,1); // generate noise term
                  //  ouProcess[val] = ouProcess[val] + (1/tauOU) *(meanOU - ouProcess[val])*dt+(sigmaOU/tauOU)*dW; // update OU term, saves update of neuron j 
                 //   mexPrintf("Ou value %f, j: %d\n", ouProcess[val],j);
                    v[j]+=fmax((ouProcess[val]+Iapp[j]+Isyntot-gl[pre_pop]*(v[j]-Vleak[pre_pop])+gl[pre_pop]*DeltaT[pre_pop]*EXP((v[j]-VT[pre_pop])/DeltaT[pre_pop]))*dt/C[pre_pop],Vlb[pre_pop]-v[j]); //EIF voltage
                  ...
                  //  v[j]+=fmax((Iapp[j]+Isyntot-gl[pre_pop]*(v[j]-Vleak[pre_pop])+gl[pre_pop]*DeltaT[pre_pop]*EXP((v[j]-VT[pre_pop])/DeltaT[pre_pop]))*dt/C[pre_pop],Vlb[pre_pop]-v[j]); //EIF voltage
                   // if (j==Nsum[popwithinput-2]+5){ // record last neuron in popwithinput population 
                   //     recordedOU[i-1]=ouProcess[val];
                   //     recordedInput[i-1] = v[j];
                   //     recordedRandNum[i-1] = normal_random_number(0,1);
                   // }
                 // mxFree(val);
                } else {
                    v[j]+=fmax((Iapp[j]+Isyntot-gl[pre_pop]*(v[j]-Vleak[pre_pop])+gl[pre_pop]*DeltaT[pre_pop]*EXP((v[j]-VT[pre_pop])/DeltaT[pre_pop]))*dt/C[pre_pop],Vlb[pre_pop]-v[j]); //EIF voltage
                    // if (j==1){
                    //     recordedInputNone[i-1] = v[j];
                    // }
                }
                 //v[j]+=fmax((Iapp[j]+Isyntot-gl[pre_pop]*(v[j]-Vleak[pre_pop])+gl[pre_pop]*DeltaT[pre_pop]*EXP((v[j]-VT[pre_pop])/DeltaT[pre_pop]))*dt/C[pre_pop],Vlb[pre_pop]-v[j]); //EIF voltage
            }
