/* poisson input rate of different values for each population and connectivity strength is Jext is 1x4;
each pop recieves a poission input rate and connecctivity strength - input includes 

NEW VERSION = EIF_MultiPop_Poisson*/
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double dt,*s,*v,*v0,*Isynrecord, *Psyn, Isyn_tmp;
  ...
  /*  mexPrintf("Kxsum=%d, Kmax=%d \n",Kxsum,Kmax);  */
    ...
    for(i=1;i<Nt && ns<maxns;i++){
        ...
        for(jj=0;jj<N*Nsyn;jj++){
            ...
            Isynprime[jj]-=dt*(Isynprime[jj]*temp1[isyn]+Isyn_tmp*temp2[isyn]);
       ...
       // mexPrintf("%d",Nsrr);
        ...
        // Isynprime=mxMalloc(Nsyn*N*sizeof(double)); pop by num of synapse
        ...
        for(j=0;j<N;j++){
            ...
            if(refstate[j]<=0){
                ...
                v[j]+=fmax((Iapp[pre_pop]+Isyntot-gl[pre_pop]*(v[j]-Vleak[pre_pop])+gl[pre_pop]*DeltaT[pre_pop]*EXP((v[j]-VT[pre_pop])/DeltaT[pre_pop]))*dt/C[pre_pop],Vlb[pre_pop]-v[j]);} //EIF voltage
            else{
                if(refstate[j]>1)
